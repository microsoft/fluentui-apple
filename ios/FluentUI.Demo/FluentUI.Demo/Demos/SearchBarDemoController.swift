...
class SearchBarDemoController: DemoController, SearchBarDelegate {
    ...
    @objc private func updateSearchbars() {
        if segmentedControl.selectedSegmentIndex == 1 {
            searchBarsStackView.backgroundColor = NavigationBar.backgroundColor(forStyle: .primary, theme: view.fluentTheme)
            updateSearchBarsStyles(to: .lightContent)
        } else {
            searchBarsStackView.backgroundColor = NavigationBar.backgroundColor(forStyle: .system, theme: view.fluentTheme)
