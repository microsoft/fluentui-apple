...
class NotificationViewDemoController: DemoController {
    ...
    private func createNotificationView(forVariant variant: Variant) -> NotificationView {
        switch variant {
        case .primaryToast:
            return NotificationView.init(style: .primaryToast).setup(style: .primaryToast,
        ...
        case .neutralToast:
            return NotificationView.init(style: .neutralToast).setup(style: .neutralToast,
                              message: "Some items require you to sign in to view them",
                              actionTitle: "Sign in",
                              action: { [weak self] in self?.showMessage("`Sign in` tapped") })
        case .dangerToast:
            return NotificationView.init(style: .dangerToast).setup(style: .dangerToast,
                              message: "There was a problem, and your recent changes may not have saved",
                              actionTitle: "Retry",
                              action: { [weak self] in self?.showMessage("`Retry` tapped") })
        case .warningToast:
            return NotificationView.init(style: .warningToast).setup(style: .warningToast,
                              message: "Read Only",
                              action: { [weak self] in self?.showMessage("`Dismiss` tapped") })
        case .primaryBar:
            return NotificationView.init(style: .primaryBar).setup(style: .primaryBar,
                              message: "Updating...")
        case .primaryOutlineBar:
            return NotificationView.init(style: .primaryOutlineBar).setup(style: .primaryOutlineBar,
                              message: "Mail Sent")
        case .neutralBar:
            return NotificationView.init(style: .neutralBar).setup(style: .neutralBar,
