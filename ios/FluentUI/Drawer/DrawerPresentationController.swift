...
class DrawerPresentationController: UIPresentationController {
    ...
    init(presentedViewController: UIViewController,
         ...
         drawerTokenResolver: TokenResolver<DrawerController>) {
        sourceViewController = source
        self.presentationDirection = presentationDirection
        self.drawerTokenResolver = drawerTokenResolver
    ...
    private lazy var shadowView: DrawerShadowView = {
        ...
        return DrawerShadowView(shadowDirection: actualPresentationOffset == 0 ? presentationDirection : nil, tokenResolver: drawerTokenResolver)
    }()
    ...
    func updateApperance() {
        ...
        dimmingView.dimmedBlackColor = UIColor(dynamicColor: drawerTokenResolver.value(\.backgroundDimmedColor))
    ...
    private func marginsForContentView() -> UIEdgeInsets {
        ...
        case .down:
            ...
            margins.bottom = max(drawerTokenResolver.value(\.minVerticalMargin), containerView.safeAreaInsets.bottom)
        case .up:
            margins.top = max(drawerTokenResolver.value(\.minVerticalMargin), containerView.safeAreaInsets.top)
        ...
        case .fromLeading:
            ...
            margins.right = max(drawerTokenResolver.value(\.minHorizontalMargin), containerView.safeAreaInsets.right)
        case .fromTrailing:
            margins.left = max(drawerTokenResolver.value(\.minHorizontalMargin), containerView.safeAreaInsets.left)
    ...
    private func setPresentedViewMask() {
        ...
        presentedView?.layer.cornerRadius = drawerTokenResolver.value(\.cornerRadius)
    }
