...
public final class MSBarButtonItems: NSObject {
    public static func confirm(target: Any?, action: Selector?) -> UIBarButtonItem {
        let image = UIImage.staticImageNamed("checkmark-blue-25x25")?.withRenderingMode(.alwaysTemplate)
        let landscapeImage = UIImage.staticImageNamed("checkmark-blue-thin-20x20")?.withRenderingMode(.alwaysTemplate)
